...
from .forms import PersonaForm , BodegaForm, Rol_personaForm, Categoria_productoForm, Bodega_productoForm, \
    ...
    ProductoForm, ProveedorForm, Unidad_medidaForm, CiudadForm
from .models import Persona, rol_persona, bodega, Categoria_producto, Bodega_producto, \
    ...
    proveedor, unidad_medida, ciudad
...
def consultar_ciudad(request):
    ciudad1 = ciudad.objects.all()
    return render(request, "ciudad/consultar_ciudad.html",{'ciudad_ls': ciudad1})
...
def crear_ciudad(request):
    if request.method == "POST":
        ...
        if ciudadForm.is_valid():
         ...
         ciudadForm = CiudadForm ()
    return render(request, "ciudad/crear_ciudad.html",{'ciudad_ls': ciudadForm})

def eliminar_ciudad(request,id):
    if request.method == "POST":
        ciudad1 = get_object_or_404(proveedor, pk=id)
        ...
        ciudad1 = get_object_or_404(proveedor, pk=id)
        ciudadForm = CiudadForm(request.POST or None, instance=ciudad1)
    return render(request, "ciudad/eliminar_ciudad.html",{'proveedor_ls': ciudadForm})
...
def modificar_ciudad(request,id):
    if request.method == "POST":
        ...
        ciudad1 = get_object_or_404(ciudad, pk=id)
        ciudadForm = CiudadForm(request.POST or None, instance=ciudad1)
    return render(request, "ciudad/modificar ciudad.html",{'ciudad_ls': ciudadForm})
